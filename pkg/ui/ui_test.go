...
func TestModelTunnelRunningView(t *testing.T) {
	t.Run("Port 22 (SSH)", func(t *testing.T) {
		...
		if !strings.Contains(view, "ssh -o \"ProxyCommand=cloudflared access tcp --hostname %h\" user@test-tunnel.trycloudflare.com") {
			t.Errorf("View missing SSH command, got:\n%s", view)
		}
		if !strings.Contains(view, "StrictHostKeyChecking=accept-new") {
			t.Errorf("View missing StrictHostKeyChecking tip, got:\n%s", view)
		}
	})
	...
	t.Run("Port 8080 (TCP)", func(t *testing.T) {
		...
		if !strings.Contains(plainView, "cloudflared access tcp --hostname test-tunnel.trycloudflare.com --url 127.0.0.1:8080") {
			t.Errorf("View missing TCP forward command, got:\n%s", plainView)
		}
